...
#DECLARATION OF INODE DATA STRUCTURE
class Inode():
    def __init__(self, type):
        self.type = type        # 0-FILE, 1-DIRECTORY
        self.blk_numbers = [-1] * (config.TOTAL_NO_OF_BLOCKS / config.MAX_NUM_INODES)   #MAPPING TABLE FROM INDEX TO ACTUAL BLOCK NUMBERS
        self.directory = dict()                                                         #MAPPING TABLE OF FILENAMES : INODE NUMBERS
        self.time_created = datetime.datetime.now()                                     #SIZE OF MAPPING ARRAY IS DECIDED ACCORDING TO CONFIG PARAMETERS
...
class InodeLayer():
    ...
    def free_invalid_data_block(self, inode, index):
        for i in range(index, len(inode.blk_numbers)):
            interface.free_invalid_data_block(inode.blk_numbers[i])
    ...
    def write(self, inode, offset, data):
        ...
        self.free_invalid_data_block(inode, index)  #INVALIDIATING ALL BLOCKS INCLUDING AND AFTER OFFSET BLOCK
