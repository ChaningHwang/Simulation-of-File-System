...
'''  
import config, DiskLayout3


#POINTER TO DISK
sblock = DiskLayout3.SuperBlock()			 
...
class Initialize():
	def __init__(self):
		...
		for i in range(0, sblock.TOTAL_NO_OF_BLOCKS / sblock.BLOCK_SIZE):  	
			sblock.ADDR_BITMAP_BLOCKS.append(DiskLayout3.Bitmap_Block(sblock.BLOCK_SIZE))
			count += 1
		...
		for i in range(0, (sblock.MAX_NUM_INODES * sblock.INODE_SIZE) / sblock.BLOCK_SIZE):		#for Inode blocks
			sblock.ADDR_INODE_BLOCKS.append(DiskLayout3.Inode_Block(sblock.INODES_PER_BLOCK))
			count  += 1
		#ALLOCATING DATA BLOCKS
		sblock.DATA_BLOCKS_OFFSET = count
		for i in range(sblock.DATA_BLOCKS_OFFSET, sblock.TOTAL_NO_OF_BLOCKS):
			sblock.ADDR_DATA_BLOCKS.append(DiskLayout3.Data_Block(sblock.BLOCK_SIZE))
